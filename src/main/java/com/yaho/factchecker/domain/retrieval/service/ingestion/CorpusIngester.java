...
public interface CorpusIngester {
     ...
     * 대상 API가 오래된 순 정렬이므로, 마지막 페이지부터 recentPages 페이지만 조회한다.
     * numOfRows = 페이지당 조회 수
    ...
    /**
     * CLEAN/WRITE 실패 재처리 — 실패 기록의 원본 item JSON 하나를 복원해 ingestOne 재실행
    ...
    /**
     * FETCH 실패 재처리 — 저장된 FetchRequest 스냅샷으로 해당 페이지를 다시 조회해 적재
     * fetchRequestJson = ingestion_failure 에 저장된 FetchRequest JSON 스냅샷
     * 반환 = 저장된 fact 수 (재처리 성공 시). 실패하면 예외를 던져 호출측이 처리
     */
    int retryFetch(String fetchRequestJson);
}
